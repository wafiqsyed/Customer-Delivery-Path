...
public class CellComponent extends JComponent {
	...
	protected void paintComponent(Graphics g) {
		...
		if (c == CellColors.BLOCK) {
			...
			displayed = true;
		} else if (c == CellColors.ODD_SWITCH || c == CellColors.ODD_SWITCH_PUSHED || c == CellColors.ODD_SWITCH_POPPED) {
			...
			} else if (c == CellColors.ODD_SWITCH_PUSHED) {
			...
			g2d.drawImage(img, 0, 0, width, width, null);
			displayed = true;
		} else if (c == CellColors.EVEN_SWITCH || c == CellColors.EVEN_SWITCH_PUSHED
				|| c == CellColors.EVEN_SWITCH_POPPED) {
			...
			} else if (c == CellColors.EVEN_SWITCH_PUSHED) {
		...
		else if (c == CellColors.OMNI_SWITCH || c == CellColors.OMNI_SWITCH_PUSHED
				|| c == CellColors.OMNI_SWITCH_POPPED) {
			// High altitude cells: initial state, when they are visited for the
			// first time,
			// and when the program decides not to include them in the path
			Image img;
			...
			} else if (c == CellColors.OMNI_SWITCH_PUSHED) {
				g2d.setColor(new Color(106, 131, 136));
				g2d.fillPolygon(hexagon);
